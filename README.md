...
「もしも自分やペットが格ゲー参戦したら？」を写真1枚で楽しむWebツール。端末内AIで被写体を高精度に抽出して食らい判定（青）を自動生成する。攻撃（赤）/ 投げ（紫）/ 押し合い（緑）の手描き追加、背景変更、合成PNGの保存・コピー・共有に対応。
...
4. モバイルは「写真に保存」から共有シートを開き、「画像を保存」でカメラロールへ追加。PCはPNG保存・コピーに対応

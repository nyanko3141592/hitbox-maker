...
通常の判定生成とスマホ版ではAIモデルをダウンロードしない。高速マスクで背景変更まで端末内で処理する。PCで任意の「高精度AI」を使う場合だけ約42MBの軽量QUINT8モデルを取得し、WebGPUまたはCPU/WASMで解析する。編集画像はPCで長辺2048px、スマホで1280pxを上限に端末内で自動縮小し、スマホのドラッグ描画はフレーム単位でまとめて処理する。

...
初回に約42MBの軽量QUINT8モデルを取得し、対応ブラウザではWebGPU、非対応時はCPU/WASMで必ず被写体を抽出する。モデルはブラウザにキャッシュされる。編集画像はPCで長辺2048px、スマホで1280px（低性能端末は1024px）を上限に端末内で自動縮小する。スマホのドラッグ描画はフレーム単位でまとめ、元画像と背景の合成結果をキャッシュする。

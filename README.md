...
画像を入れるだけで、格ゲー風の当たり判定が付く Web ツール。モデル不要の高速解析で食らい判定（青）を即生成し、必要な場合だけ端末内AIで高精度化できる。攻撃（赤）/ 投げ（紫）/ 押し合い（緑）の手描き追加、背景変更、合成PNGの保存・コピー・共有に対応。

画像の解析はすべて端末内（WASM）で完結し、サーバーには一切送信されない。
通常の判定生成とスマホ版ではAIモデルをダウンロードしない。高速マスクで背景変更まで端末内で処理する。PCで任意の「高精度AI」を使う場合だけ約42MBの軽量QUINT8モデルを取得し、WebGPUまたはCPU/WASMで解析する。編集画像はPCで長辺2048px、スマホで1280px（低性能端末は1024px）を上限に端末内で自動縮小する。スマホのドラッグ描画はフレーム単位でまとめ、元画像と背景の合成結果をキャッシュする。

## 使い方

1. トップ画面に画像をドロップ（タップ選択 / ⌘V ペーストも可）
2. 高速解析が食らい判定ボックスを即生成（「判定付与!!」）
